...
public class SolarPanel8 : SolarPanelBase // Inherits from SolarPanelBase
{
    public Light sun;
    ...
    protected override void UpdatePowerOutput()
    {
        ...
        if (sunElevationSine > 0)
        {
            ...
            powerOutputText.text = ($"Panel-8: {currentPowerOutput.ToString("F2")} Watts");
        }
        else
        {
            currentPowerOutput = 0;
            powerOutputText.text = ($"Panel-8: {currentPowerOutput.ToString("F2")} Watts"); // Sun is below the horizon
        }
    }
