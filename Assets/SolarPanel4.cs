...
public class SolarPanel4 : SolarPanelBase // Inherits from SolarPanelBase
{
    public Light sun;
    ...
    void Update()
    {
        ...
        UpdateBattery(); // Update the battery manager
    }
    ...
    protected override void UpdatePowerOutput()
    {
        ...
        if (sunElevationSine > 0)
        {
            ...
            powerOutputText.text = ($"Panel-4: {currentPowerOutput.ToString("F2")} Watts");
        }
        else
        {
            currentPowerOutput = 0;
            powerOutputText.text = ($"Panel-4: {currentPowerOutput.ToString("F2")} Watts"); // Sun is below the horizon
        }
    }
    ...
    private float GetRainEffect()
    {
        ...
        if (rainParticleSystem.gameObject.activeInHierarchy)
        {
            ...
            float rainImpact = Mathf.Clamp01(rainParticleSystem.emission.rateOverTime.constant / 1000000);
    ...
    private float GetCloudEffect()
    {
        ...
        {
            float cloudImpact = Mathf.Clamp01(cloudParticleSystem.emission.rateOverTime.constant / 1000000);
