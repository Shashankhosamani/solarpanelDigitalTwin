...
public class SolarPanel1 : SolarPanelBase // Inherits from SolarPanelBase
{
    public Light sun;
    ...
    protected override void UpdatePowerOutput()
    {
        ...
        if (sunElevationSine > 0)
        {
            ...
            powerOutputText.text = ($"Panel-1: {currentPowerOutput.ToString("F2")} Watts");
        }
        else
        {
            currentPowerOutput = 0;
            powerOutputText.text = ($"Panel-1: {currentPowerOutput.ToString("F2")} Watts"); // Sun is below the horizon
        }
    }
